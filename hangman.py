...
def play(word):
    ...
    guessed_words = []
    tries = 6
    print(Fore.GREEN + "play hangman")
    print(Fore.RED + display_man(tries))
    print(Fore.BLUE + word_completion)
    print("\n")
    ...
    if guessed:
        print(Fore.CYAN + "You guessed the word, you win")
    else:
        print(Fore.RED + "You ran out of tries. The word was " + word)
...
def main():
    word = get_word()
    play(word)
    while input(Fore.YELLOW + "play again? (Y or N)").upper() == "Y":
        new_game()
        word = get_word()
        play(word)
